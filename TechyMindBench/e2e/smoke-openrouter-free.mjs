#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
// TechyMindBench/e2e/smoke-openrouter-free.mjs — PRE-FLIGHT SMOKE TEST for
...
//
//   OPENROUTER_API_KEY=sk-or-… node TechyMindBench/e2e/smoke-openrouter-free.mjs
//   OPENROUTER_API_KEY=sk-or-… node TechyMindBench/e2e/smoke-openrouter-free.mjs \
...
if (!KEY) {
  ...
  console.error('  OPENROUTER_API_KEY=sk-or-… node TechyMindBench/e2e/smoke-openrouter-free.mjs');
  process.exit(1);
}
...
try {
  res = await fetch(`${BASE_URL}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'OpenComet SIH TechyMindBench smoke',
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [{
...
console.log('  next         : run the full loop → node TechyMindBench/e2e/run-e2e-real.mjs \\');
