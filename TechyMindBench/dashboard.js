// ─────────────────────────────────────────────────────────────────────────────
// TechyMindBench/dashboard.js — rendering + report import for the admin monitor.
...
function renderMemo() {
  ...
  (25 checks) + <code>TechyMindBench/probe-scene-change.mjs</code> (real YOLO, wire-level probe).</div>
...
function renderE2e() {
  ...
  if (currentE2eReal) {
    ...
    <table><thead><tr><th>Provider</th><th>Command</th></tr></thead><tbody>
      <tr><td>Local (Ollama)</td><td><code>node TechyMindBench/e2e/run-e2e-real.mjs --provider=ollama --model=qwen2.5v:7b</code></td></tr>
      <tr><td>Remote (OpenAI-compatible)</td><td><code>node TechyMindBench/e2e/run-e2e-real.mjs --provider=openai --model=gpt-4o-mini --api-key=…</code></td></tr>
    </tbody></table>
