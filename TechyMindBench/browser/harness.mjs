#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
// TechyMindBench/browser/harness.mjs — SIH v1.13 REAL BROWSER BENCHMARK
//
...
//
//   node TechyMindBench/browser/harness.mjs              # all tiers
//   node TechyMindBench/browser/harness.mjs --only=visual,redaction,ocr,resources
//   node TechyMindBench/browser/harness.mjs --vit=false  # skip the ViT download
//   node TechyMindBench/browser/harness.mjs --headed --channel=chrome  # REAL HARDWARE MODE
...
//
// Output: TechyMindBench/results/browser-benchmark.json (meta.type = "browser").
...
function serve() {
  return new Promise((res) => {
    const srv = createServer((req, resp) => {
      ...
      if (p === '/') p = '/TechyMindBench/browser/runner.html';
...
try {
  ...
  if (TIERS.has('visual')) {
    ...
    if (VIT_ENABLED) {
      ...
      await vitPage.goto(`${BASE}/TechyMindBench/pages/login.html`, { waitUntil: 'load' });
    ...
    for (const name of VISUAL_PAGES) {
      ...
      const page = await openPage(ctx, `${BASE}/TechyMindBench/pages/${name}`);
      ...
      for (const zoom of ZOOMS) {
        for (const scroll of SCROLLS) {
          for (const dialogOpen of DIALOGS) {
            const page = await openPage(ctx, `${BASE}/TechyMindBench/pages/redaction-lab.html`);
  ...
  if (TIERS.has('ocr')) {
    ...
    const page = await openPage(ctx, `${BASE}/TechyMindBench/pages/pii-visual.html`);
    const shot = await page.screenshot({ type: 'png' });
    const imageDataUrl = `data:image/png;base64,${shot.toString('base64')}`;
