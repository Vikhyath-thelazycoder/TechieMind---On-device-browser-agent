...
to leak a password, an Aadhaar number, a card, or a face. TechyMind-Bench is
the measurement instrument built to prove — case by case, byte by byte —
that TechyMind's perception-to-network path keeps those promises under active
adversarial pressure.

It is not a test folder that happens to exist. It is an **open
...
`TechyMindBench/results/` or to a runnable command in §11. No number on this page
...
Run with `node TechyMindBench/run-all.js` (human table) or `--json` (feeds the
in-app SIH Scorecard). Six suites, all importing the *shipped* extension
modules. Latest measured run: `TechyMindBench/results/unit-suite-latest.json`
(2026-09-06).

### OCB-U1 · PII detection — 514-case checksum-validated corpus

The corpus (`TechyMindBench/fixtures/pii-corpus.json`) holds **366 positives
...
`node TechyMindBench/browser/harness.mjs` renders the 16 synthetic fixture pages
(`TechyMindBench/pages/`, generated by `generate-pages.mjs` with embedded ground
...
`node TechyMindBench/e2e/run-e2e.mjs` loads the **unpacked extension**, points
...
(`TechyMindBench/e2e/adversarial/`, regenerated by
...
node TechyMindBench/run-all.js              # human-readable table
node TechyMindBench/run-all.js --json       # machine-readable → SIH Scorecard

# Tier B — BROWSER (real pixels; headed = your GPU, production-claim mode)
node TechyMindBench/browser/harness.mjs                         # headless (CI label)
node TechyMindBench/browser/harness.mjs --headed --channel=chrome  # real hardware
node TechyMindBench/browser/harness.mjs --only=visual,ocr --vit=false
...
node TechyMindBench/e2e/run-e2e-real.mjs --provider=ollama --model=<vision-model>

# Tier D — ADVERSARIAL (recording wire server)
node TechyMindBench/e2e/run-adversarial.mjs                 # both suites
node TechyMindBench/e2e/run-adversarial.mjs --suite=privacy # or =injection
...
node TechyMindBench/generate-pages.mjs
node TechyMindBench/generate-pii-corpus.mjs
node TechyMindBench/e2e/generate-adversarial-pages.mjs
```

All results land in `TechyMindBench/results/<type>-benchmark-<timestamp>.json`.
...
- **New PII type**: add positives/negatives to
  `TechyMindBench/fixtures/pii-corpus.json` (positives `{type, text, expect}`,
...
- **New exfiltration channel**: append one
  ...
  `TechyMindBench/fuzz.test.js` — the matrix grows by 8×3 cases and the 216
  headline re-counts itself.
- **New hostile page**: drop an HTML fixture in
  `TechyMindBench/e2e/adversarial/` (or regenerate via
...
- **New browser condition**: add a DPR/zoom/scroll cell to the matrix in
  `TechyMindBench/browser/harness.mjs`.
