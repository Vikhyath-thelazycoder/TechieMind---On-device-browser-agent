#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
// TechyMindBench/probe-scene-change.mjs — v1.14.1 SCENE-CHANGE ATTACK PROBE
...
// Result: TechyMindBench/results/scene-change-probe-<ts>.json
// ─────────────────────────────────────────────────────────────────────────────
...
await p.goto('http://127.0.0.1:8896/TechyMindBench/pages/login.html', { waitUntil: 'load' });
...
mkdirSync(`${ROOT}/TechyMindBench/results`, { recursive: true });
const ts = Date.now();
writeFileSync(`${ROOT}/TechyMindBench/results/scene-change-probe-${ts}.json`, JSON.stringify(result, null, 2));
console.log(`\nwrote TechyMindBench/results/scene-change-probe-${ts}.json  PASS=${fail === 0}`);
