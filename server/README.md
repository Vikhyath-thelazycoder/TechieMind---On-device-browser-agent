...
Privacy-aware companion server for the TechyMind browser agent.
...
# Option A: Use a local Ollama model (fully offline)
OLLAMA_BASE_URL=http://localhost:11434 node server.js
# Then in the extension settings, choose provider=ollama, model=qwen2.5:7b
...
## Redaction Awareness & Privacy Architecture

The server is intentionally aware of the redaction manifest. The prompt sent to the VLM includes:
...
This lets the model reason about the page (`"there's a password field, so this is a login form"`) **without ever seeing the actual password**. The client-side redaction + server-side manifest together form the privacy contract.

## Redaction tokens in DOM text

Whenever the client-side regex/NER pipeline finds PII in the DOM text, it replaces the raw value with a `[REDACTED:<type>]` token, e.g.:
...
The server's prompt explains this convention so the VLM treats the tokens as opaque placeholders.
...
The local vision pipeline adds <100 ms on WebGPU and pays for itself by eliminating the need for any cloud-side privacy filtering.
