// server/server.js
// Companion server for TechyMind Privacy Vision Agent.
//
// Responsibilities:
...
const SHARED_TOKEN = process.env.TECHYMIND_TOKEN || process.env.OPENCOMET_TOKEN || '';
app.use('/agent/decide', (req, res, next) => {
  if (rateLimited(req.ip || 'unknown')) {
    return res.status(429).json({ error: 'Rate limit exceeded — slow down.' });
  }
  if (SHARED_TOKEN) {
    const got = String(req.headers['x-techymind-token'] || req.headers['x-opencomet-token'] || '');
    if (got !== SHARED_TOKEN) {
      return res.status(401).json({ error: 'Missing or wrong authentication token header.' });
    }
  }
  next();
});
  ...
  try {
    ...
    if (!SHARED_TOKEN && !settings.apiKey && envKeyBackends.includes(backend)) {
      return res.status(403).json({
        ok: false,
        error: 'Refusing to spend operator-provided keys on an unauthenticated request. Set X-TechyMind-Token (server env TECHYMIND_TOKEN) or send your own settings.apiKey.',
      });
    }
...
app.post('/vision/describe', upload.single('image'), async (req, res) => {
  try {
    ...
    if (process.env.TECHYMIND_ALLOW_RAW_VISION !== '1' && process.env.OPENCOMET_ALLOW_RAW_VISION !== '1') {
      return res.status(403).json({
        ok: false,
        error: 'Disabled: /vision/describe accepts raw images and is local-test-only. Start the server with TECHYMIND_ALLOW_RAW_VISION=1 to enable it for local diagnostics.',
      });
    }
...
async function callOllama(settings, model, prompt, imageBuffer) {
  ...
  const m = model || defaultModelFor('ollama');
  const b64 = imageBuffer ? imageBuffer.toString('base64') : '';
  const isVision = supportsOllamaVision(m);
...
app.listen(PORT, HOST, () => {
  console.log(`\n  ┌─────────────────────────────────────────────────────────┐`);
  console.log(`  │  TechyMind Privacy Vision Server                        │`);
