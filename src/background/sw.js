...
console.log(`[TechyMind] v${(chrome.runtime && typeof chrome.runtime.getManifest === 'function') ? chrome.runtime.getManifest().version : 'dev'} · service worker loaded`);
...
(async () => {
  try {
    ...
    console.warn('[TechyMind] Crash recovery: previous run', run.sessionId, 'was interrupted by a service-worker restart — finalized in History.');
  } catch (err) {
    console.warn('[TechyMind] Crash-recovery reconciliation failed (non-fatal):', err?.message || err);
  }
})();
