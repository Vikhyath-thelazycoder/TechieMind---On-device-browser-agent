...
async function toggleModelSelector() {
  ...
  dropdown.innerHTML = items.map((it, idx) => `
    <div class="model-opt-item${it.selected ? ' selected' : ''}" data-idx="${idx}">
      <div style="display:flex;flex-direction:column">
        <span style="font-weight:600">${esc(it.label)}</span>
        <span style="font-size:11px;color:var(--tx3)">${esc(it.desc)}</span>
      </div>
  ...
  dropdown.querySelectorAll('.model-opt-item').forEach(el => {
...
function setInputTab(tab) {
  ...
  const modeLabel = $('modeCompactLabel');
  if (modeLabel) {
    modeLabel.textContent = isResearch ? 'Deep Search' : (isScrape ? 'Scrape' : 'Search');
  }
  const modeDropdown = $('modeCompactDropdown');
  if (modeDropdown) modeDropdown.classList.remove('open');
...
const modeCompactBtn = $('modeCompactBtn');
const modeCompactDropdown = $('modeCompactDropdown');
if (modeCompactBtn && modeCompactDropdown) {
  modeCompactBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    modeCompactDropdown.classList.toggle('open');
  });
  document.addEventListener('click', (e) => {
    if (!modeCompactDropdown.contains(e.target) && !modeCompactBtn.contains(e.target)) {
      modeCompactDropdown.classList.remove('open');
    }
  });
}

// Quick-start suggestion chips & cards (empty state)
// Event-delegated on document so handlers survive resetConversationUI()'s
// innerHTML restore of the empty state markup.
document.addEventListener('click', e => {
  const chip = e.target.closest('.suggest-chip, .suggest-card');
  if (!chip) return;
...
function sanitizeStepText(type, rawText) {
  if (!rawText || typeof rawText !== 'string') return rawText || '';
  const text = rawText.trim();
  if (type === 'thinking' || type === 'spin') {
...
function addStep(type, text, meta = null) {
  ...
  const displayText = sanitizeStepText(type, text);
  const chipHtml = stepTimeChipHtml(meta);
  row.innerHTML = `${iconHtml}<span class="${textClass}">${esc(displayText)}</span>${chipHtml}`;
...
async function loadSkills() {
  ...
  const seen = new Set();
  // User skills take precedence so customized prompts are respected!
  allSkillsCache = [...userSkills, ...builtinMeta].filter(s => {
...
async function materializeSkillById(id) {
  ...
  const stored = await getStoredSkillById(id);
...
function renderSkillCard(skill) {
  ...
  const isBuiltIn  = skill.builtIn && skill.source !== 'user';
  const checklist  = (skill.doneChecklist || [])
    .map(c => `<div class="skill-checklist-item">${esc(c)}</div>`).join('');
  const hosts      = (skill.allowedHosts || [])
    .map(h => `<span class="skill-host-chip">${esc(h)}</span>`).join('');
  const fullPrompt = skill.prompt || skill.promptPreview || '';

  return `
    <div class="skill-card${isActive ? ' is-active' : ''}" data-id="${esc(skill.id)}" data-name="${esc(skill.name)}">
      <div class="skill-card-head" title="Click to inspect injected prompt &amp; details">
        ...
        ${skill.source === 'library' ? '<span class="skill-builtin-badge">Library</span>' : isBuiltIn ? '<span class="skill-builtin-badge">Built-in</span>' : '<span class="skill-builtin-badge" style="color:var(--accent)">Custom</span>'}
        <button class="skill-toggle${isActive ? ' on' : ''}" title="${isActive ? 'Deactivate' : 'Activate'}"></button>
      </div>
      <div class="skill-card-body">
        <div class="skill-prompt-section">
          <div class="skill-prompt-label">Injected Agent Instructions &amp; Prompt:</div>
          <div class="skill-prompt-preview">${esc(fullPrompt)}</div>
        </div>
        ${checklist ? `
          <div class="skill-prompt-label">Validation Checklist:</div>
          <div class="skill-checklist">${checklist}</div>
        ` : ''}
        ${hosts ? `
          <div class="skill-prompt-label">Allowed Domains:</div>
          <div class="skill-hosts">${hosts}</div>
        ` : ''}
        <div class="skill-actions">
          <button class="skill-action-btn primary" data-action="edit">Edit Instructions</button>
          <button class="skill-action-btn" data-action="use">${isActive ? 'Active' : 'Use now'}</button>
...
(function privacyModeController() {
  ...
  console.log('[TechyMind] Privacy Mode controller initialised. Enabled:', privacyEnabled);
})();
...
console.log(`[TechyMind] v${_ocV} · side panel`);
...
(function aboutPageController() {
  ...
  if (copyBtn && copyState) {
    copyBtn.addEventListener('click', async () => {
      const text = [
        'TechyMind — Privacy-First Browser Agent',
        `Version: ${_ocV}`,
      ...
      try {
        ...
        console.log('[TechyMind] build info\n' + text);
      }
