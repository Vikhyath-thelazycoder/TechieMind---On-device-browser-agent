...
export function getProviderCapabilities(settings = {}) {
  ...
  const registry = {
    ...
    ollama: {
      vision:           supportsOllamaVision(ollamaVisionModel || (supportsOllamaVision(ollamaTextModel) ? ollamaTextModel : '')),
      json:             true,
...
function logVlmRequest(tag, prompt, images, options = {}) {
  try {
    ...
    console.groupCollapsed(`[TechyMind][VLM-REQ] → ${tag} · prompt ${prompt?.length ?? 0} chars · ${imgs.length} image(s)`);
...
function logVlmResponse(tag, out, ms) {
  try {
    console.groupCollapsed(`[TechyMind][VLM-RES] ← ${tag} · ${ms}ms`);
...
function logVlmRawText(tag, content) {
  try {
    console.groupCollapsed(`[TechyMind][VLM-RAW] ← ${tag} · ${String(content || '').length} chars (model text before JSON parsing)`);
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (attempt.label === 'vision' && isRecoverableMistralImageError(err)) {
        console.warn(`[TechyMind] Mistral vision attempt failed, falling back to ${attempts[attempts.indexOf(attempt) + 1]?.label || 'next'}:`, err.message);
        continue;
      }
      break; 
    }
